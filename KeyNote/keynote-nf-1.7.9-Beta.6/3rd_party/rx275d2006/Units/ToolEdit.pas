...
{$ENDIF RX_D4}
    ...
    try
      ...
      with ACanvas do begin
        ...
        S := AText;
        AWidth := TextWidth(S);
        MarginsPnt := EditorTextMargins(Editor);
        if PopupVisible then ALeft := MarginsPnt.X
        else begin
          ...
          case AAlignment of
            taLeftJustify:
              ALeft := MarginsPnt.X;
            taRightJustify:
              ALeft := ClientWidth - ButtonWidth - AWidth - MarginsPnt.X - 2;
            else
...
{$ENDIF}
        TextRect(R, ALeft, MarginsPnt.Y, S);
      end;
    finally
      ACanvas.Handle := 0;
